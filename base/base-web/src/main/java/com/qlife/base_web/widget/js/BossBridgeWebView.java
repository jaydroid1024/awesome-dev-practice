...
public class BossBridgeWebView extends BaseX5WebView implements WebViewJavascriptBridge {

  public static String TAG = "BridgeWebView";
  ...
  public BossBridgeWebView(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public BossBridgeWebView(Context context, AttributeSet attrs, int defStyle) {
  ...
  private void initJsWeb() {
    ...
    setLeftToRightListener();
    this.setWebViewClient(generateBridgeWebViewClient());
  }

  /** 设置右滑事件 */
  private void setLeftToRightListener() {
    setOnTouchListener(
        (v, event) -> {
          switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
              ...
              // 在手机左端右滑150像素
              if (startX < 100) {
                if (Math.abs(endX - startX) > 250
                    && ((endY - startY) == 0 || Math.abs((endX - startX) / (endY - startY)) > 2)) {
                  listener.onSwipLefted();
                }
              }
              break;
            default:
  ...
  /**
   * register handler,so that javascript can call it 注册处理程序,以便javascript调用它
   *
   * @param handlerName handlerName
  ...
  /**
   * call javascript registered handler 调用javascript处理程序注册
   *
   * @param handlerName handlerName
   * @param data data
  ...
  public void setWebViewClientListener(WebViewClientListener listener) {
    ...
    bridgeWebViewClient.setWebViewClientListener(listener);
  }
}
