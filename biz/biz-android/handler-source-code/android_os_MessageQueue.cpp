...
namespace android {
static struct {
    jfieldID mPtr;   // native object attached to the DVM MessageQueue jfieldID 指针；附加到 DVM MessageQueue 的本机对象
...
static const int CALLBACK_EVENT_INPUT = 1 << 0; //1
static const int CALLBACK_EVENT_OUTPUT = 1 << 1; //2
static const int CALLBACK_EVENT_ERROR = 1 << 2; //4l
...
static jlong android_os_MessageQueue_nativeInit(JNIEnv* env, jclass clazz) {
    ...
    if (!nativeMessageQueue) { //非零为 true
        jniThrowRuntimeException(env, "Unable to allocate native queue"); //无法分配本机队列
        return 0;
    }
    // todo incStrong
