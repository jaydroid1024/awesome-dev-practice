...
import com.jay.lib_net.app.LibNetApp
...
abstract class AbstractNetwork<T> : BaseNetwork<T>() {
    ...
    override fun okHttpClientBuilderHandler(builder: OkHttpClient.Builder): OkHttpClient.Builder {
        if (BuildConfig.DEBUG) {
            ...
            //会将http信息发送到push页面
            builder.addInterceptor(ChuckInterceptor(LibNetApp.getApp()))
        }
        builder.addInterceptor(NetworkAvailabilityInterceptor(LibNetApp.getApp()))
